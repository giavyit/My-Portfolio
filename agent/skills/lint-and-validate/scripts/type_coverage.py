...
def check_typescript_coverage(project_path: Path) -> dict:
    ...
    for file_path in ts_files[:30]:  # Limit
        try:
            ...
            # Modern React components inference
            if '.tsx' in str(file_path):
                # Exported functions starting with a Capital letter are considered React components
                react_components = re.findall(r'export\s+(?:default\s+)?function\s+[A-Z]\w*\s*\(', content)
                typed += react_components
                stats['untyped_functions'] -= len(react_components)
                if stats['untyped_functions'] < 0:
                    stats['untyped_functions'] = 0
